    ...
    <div className="flex flex-col gap-4 text-left">
      ...
      <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-900 rounded-lg">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-bold text-gray-900 dark:text-gray-100">{tests.length}</span>
        </div>
        <div className="flex items-center gap-2">
          <svg className="w-4 h-4 text-green-500" fill="currentColor" viewBox="0 0 20 20">
            <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
          </svg>
          <span className="text-xs font-medium text-green-600 dark:text-green-400">{passed} passed</span>
        </div>
        {failed > 0 && (
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-medium text-red-600 dark:text-red-400">{failed} failed</span>
          </div>
        )}
      ...
      {/* Test Results */}
      <div>
        <h3 className="text-xs font-semibold text-gray-700 dark:text-gray-300 mb-2">Test Results</h3>
        <div className="space-y-2">
          {tests.length === 0 ? (
            <div className="text-xs text-gray-500 dark:text-gray-400">No tests executed.</div>
          ) : (
            tests.map((test, idx) => (
              <div
                key={idx}
      ...
      {consoleOutput && consoleOutput.length > 0 && (
        <div>
          <h3 className="text-xs font-semibold text-gray-700 dark:text-gray-300 mb-2">Console Output</h3>
          <div className="bg-gray-900 dark:bg-black text-gray-100 dark:text-gray-300 p-3 rounded-lg font-mono text-xs overflow-x-auto">
